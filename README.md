...
[![Tests](https://img.shields.io/badge/tests-337%20passing-brightgreen.svg)](src/tests)
...
2. **Assemble the test suite** `T = U u S` — the project's own tests plus, optionally,
   high-coverage tests generated by KLEE
...
Seven demos cover six of the paper's seven evaluation codebases (Mosquitto twice; rav1e has no
bundled demo). Each is self-contained — it clones the target at a pinned tag, builds it with the
feature on and off, runs the tests, invokes `gcov -f`, and computes `D_f` — and writes a
`manifest.json` recording the exact git commit and tool versions used.
