...
**Delimiter balance is preserved.** `D_f` contains only lines gcov attributes executable code to; a closing brace is never such a line. So `if (x) {` can enter `D_f` while its matching `}` cannot, and blanking the opener alone closes the enclosing function early. The planner (`plan_removal_detailed()`) joins runs separated only by lines that carry no code in either build, absorbs the delimiters needed to close a run, and can absorb feature-only code that `B_all` compiled but `T` never executed (it carries no code in `B_f`, so removing it changes nothing `B_f` compiles). It never absorbs a line `B_f` compiles: when closing a run would need such a line, the run is kept as a *guard of shared code* (`RemovalResult.guards_shared_code`), and a mapped line that `B_f` itself compiles (a signature whose `#else` arm is a stub) is kept the same way. A delimiter-only run that shared code needs is kept as `retained_structural`. Both kept categories are consistent with the paper's rule that unexecuted code is never removed, and they do not fail exact removal; only a run the planner can neither close nor classify is declined (`RemovalResult.skipped_unbalanced`), and that fails the run under `require_complete`.
